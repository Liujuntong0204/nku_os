...
```
               ...
               |          v                        (分配进程结构)
               |    +-------------+
               |    | PROC_UNINIT |  <- 未初始化状态，进程刚被创建
               |    +-------------+
               |          | do_fork
               |          v                        (完成fork操作)
               |    +-------------+ do_wait +-------------+
      do_wait  |    |PROC_RUNNABLE| <---->  |PROC_SLEEPING|
               |    +-------------+ wake_up +-------------+
               |          | do_exit
               |          v                        (进程主动退出)
               |    +-------------+
...
| **状态转换**                      | **触发函数**   | **触发时机**                                          |
| :-------------------------------- | -------------- | ----------------------------------------------------- |
| `none` → `PROC_UNINIT`            | `alloc_proc`   | 系统决定创建新进程时，分配进程控制块和资源。          |
| `PROC_UNINIT` → `PROC_RUNNABLE`   | `do_fork`      | 子进程完成初始化后，进入调度器的可运行队列。          |
| `PROC_RUNNABLE` → `PROC_SLEEPING` | `do_wait`      | 进程主动等待某个资源或条件（如文件 I/O 或信号）。     |
| `PROC_SLEEPING` → `PROC_RUNNABLE` | `wake_up`      | 资源条件满足时（如 I/O 完成、信号通知），进程被唤醒。 |
| `PROC_RUNNABLE` → `PROC_ZOMBIE`   | `do_exit`      | 进程完成任务并主动退出，等待父进程回收退出状态信息。  |
| `PROC_ZOMBIE` → `none`            | `do_wait` (父) | 父进程调用 `wait`，清理子进程的所有资源。             |
...
- **原理**：进程生命周期包括创建、运行、等待、退出等状态。操作系统通过进程控制块（PCB）维护进程的状态信息。
- **图示体现**：`none` 到 `PROC_ZOMBIE` 的状态切换体现了完整的进程生命周期管理。

2、**进程调度**

- **原理**：操作系统调度器根据进程状态决定是否分配 CPU。`PROC_RUNNABLE` 是调度器的关键关注状态。
- **图示体现**：`PROC_RUNNABLE` 状态与 `PROC_SLEEPING` 状态之间的切换通过 `do_wait` 和 `wake_up` 显示了调度器如何处理阻塞进程。

3、**阻塞与唤醒机制**

- **原理**：进程通过进入 `SLEEPING` 状态主动释放 CPU，等待某种外部事件（如 I/O 完成）。事件发生后，操作系统通过 `wake_up` 恢复进程。
- **图示体现**：`PROC_SLEEPING` 到 `PROC_RUNNABLE` 的切换展示了 I/O 或信号事件的完成如何恢复阻塞进程。

4、**进程终止和僵尸状态**

- **原理**：操作系统在进程结束后保留其退出信息（如退出码）供父进程读取。`PROC_ZOMBIE` 是进程退出的中间状态，防止资源泄漏。
- **图示体现**：`do_exit` 将进程置为 `PROC_ZOMBIE`，直至父进程调用 `wait` 释放资源。

5、**父子进程交互**

- **原理**：父进程通过 `fork` 创建子进程，并通过 `wait` 读取子进程退出状态。父进程对僵尸进程的管理体现了进程层次结构的重要性。
- **图示体现**：`do_wait` 处理父进程回收子进程资源的过程。
